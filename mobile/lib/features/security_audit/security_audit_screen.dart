      ...
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: const [
            ...
            Tab(text: 'Visitors', icon: Icon(Icons.person_pin_rounded, size: 15)),
...
class _NightAuditTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
          ...
          child: Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                ...
                Text('Today\'s Audit (${today.businessDate.toIso8601String().substring(0, 10)})', style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 15)),
                const Spacer(),
                Container(
              ...
              const Divider(),
              _auditRow('Cash Drops', '${today.cashDropCount} (₦${_fmt(today.cashDropTotal)})'),
              if (today.closedBy != null) _auditRow('Closed By', today.closedBy!),
              if (!locked)
                Padding(
                  padding: const EdgeInsets.only(top: 12),
                  child: SizedBox(
                    width: double.infinity,
                    child: RoleGate(requiredPermission: Permission.closeNightAudit, child: ElevatedButton.icon(
      ...
      Text('Audit History (${audits.length})', style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 15)),
      const SizedBox(height: 8),
      ...audits.map((a) => Card(child: ListTile(
        ...
        onTap: () {
          showDialog(context: context, builder: (ctx) => AlertDialog(
            title: Text('Night Audit — ${a.businessDate.toIso8601String().substring(0, 10)}'),
            content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              _row('Cash Drops', '${a.cashDropCount} (₦${_fmt(a.cashDropTotal)})'),
              if (a.closedBy != null) _row('Closed By', a.closedBy!),
  ...
  Widget _row(String label, String value) => Padding(
    ...
    child: Row(children: [Text('$label: ', style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13)), Text(value, style: const TextStyle(fontSize: 13))]),
  );
    ...
    showModalBottomSheet(context: context, isScrollControlled: true,
      ...
      builder: (ctx) => Padding(
        ...
        child: SingleChildScrollView(
          ...
          const Text('Start Night Audit', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
          const SizedBox(height: 12),
          Row(children: [
            Expanded(child: TextField(controller: roomCtl, decoration: const InputDecoration(labelText: 'Room Revenue (₦)', border: OutlineInputBorder()), keyboardType: TextInputType.number)),
            const SizedBox(width: 8),
            Expanded(child: TextField(controller: fnbCtl, decoration: const InputDecoration(labelText: 'F&B Revenue (₦)', border: OutlineInputBorder()), keyboardType: TextInputType.number)),
          ]),
          const SizedBox(height: 8),
          Row(children: [
            Expanded(child: TextField(controller: otherCtl, decoration: const InputDecoration(labelText: 'Other Revenue (₦)', border: OutlineInputBorder()), keyboardType: TextInputType.number)),
            const SizedBox(width: 8),
            Expanded(child: TextField(controller: cashCtl, decoration: const InputDecoration(labelText: 'Cash Drop Total (₦)', border: OutlineInputBorder()), keyboardType: TextInputType.number)),
          ]),
          const SizedBox(height: 12),
          SizedBox(
            width: double.infinity,
            child: ElevatedButton(
              ...
              onPressed: () {
                ...
                SecurityAuditStore.addNightAudit(NightAuditLog(
                  ...
                  totalRevenue: total, roomRevenue: room, fnbRevenue: fnb, otherRevenue: other,
                  cashDropTotal: cash, cashDropCount: cash > 0 ? 1 : 0,
                ));
                Navigator.pop(ctx);
                onChange();
              },
  ...
  void _closeAudit(BuildContext context, NightAuditLog audit) {
    ...
    showModalBottomSheet(context: context, isScrollControlled: true,
      ...
      builder: (ctx) => Padding(
        ...
        child: SingleChildScrollView(
          child: Column(mainAxisSize: MainAxisSize.min, children: [
          const Text('Close & Lock Day', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
          const SizedBox(height: 12),
          TextField(controller: nameCtl, decoration: const InputDecoration(labelText: 'Closed By', border: OutlineInputBorder())),
          const SizedBox(height: 8),
          TextField(controller: cashCtl, decoration: const InputDecoration(labelText: 'Additional Cash Drop (₦)', border: OutlineInputBorder()), keyboardType: TextInputType.number),
          const SizedBox(height: 8),
          TextField(controller: notesCtl, decoration: const InputDecoration(labelText: 'Shift Handover Notes', border: OutlineInputBorder()), maxLines: 3),
          const SizedBox(height: 12),
          SizedBox(
            width: double.infinity,
            child: ElevatedButton(
              ...
              onPressed: () {
                ...
                SecurityAuditStore.updateNightAudit(audit.id, NightAuditLog(
                  id: audit.id, businessDate: audit.businessDate,
                  totalRevenue: audit.totalRevenue, roomRevenue: audit.roomRevenue,
                  fnbRevenue: audit.fnbRevenue, otherRevenue: audit.otherRevenue,
                  ...
                  closedAt: DateTime.now(), locked: true,
                ));
                Navigator.pop(ctx);
                onChange();
              },
              ...
              : ListView.builder(padding: const EdgeInsets.all(8), itemCount: incidents.length, itemBuilder: (ctx, i) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      subtitle: Text('${inc.location ?? 'N/A'}  •  ${inc.dateReported.toIso8601String().substring(0, 10)}  •  ${inc.reportedBy ?? 'N/A'}', style: const TextStyle(fontSize: 11)),
                      trailing: RoleGate(requiredPermission: Permission.manageIncidentReports, child: PopupMenuButton<String>(
                        itemBuilder: (_) => [
                          ...
                          if (v == 'detail') {
                            showDialog(context: context, builder: (ctx) => AlertDialog(
                              ...
                              content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
                                ...
                                if (inc.location != null) _row('Location', inc.location!),
                                if (inc.description != null) _row('Description', inc.description!),
                                _row('Reported By', inc.reportedBy ?? 'N/A'),
                                _row('Date', inc.dateReported.toIso8601String().substring(0, 10)),
                                _row('Status', inc.status.name),
                                if (inc.resolvedBy != null) _row('Resolved By', inc.resolvedBy!),
                                if (inc.notes != null) _row('Notes', inc.notes!),
                              ]),
                              actions: [TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Close'))],
                            ));
                          } else if (v == 'edit') {
                            ...
                            SecurityAuditStore.updateIncident(inc.id, SecurityIncident(id: inc.id, location: inc.location, description: inc.description, reportedBy: inc.reportedBy, type: inc.type, status: IncidentStatus.investigating, dateReported: inc.dateReported));
                            onChange();
                          } else if (v == 'resolve') {
                            SecurityAuditStore.updateIncident(inc.id, SecurityIncident(id: inc.id, location: inc.location, description: inc.description, reportedBy: inc.reportedBy, type: inc.type, status: IncidentStatus.resolved, dateReported: inc.dateReported, dateResolved: DateTime.now()));
                            onChange();
                          } else if (v == 'delete') { SecurityAuditStore.removeIncident(inc.id); onChange(); }
                        },
                      )),
                    ),
                  );
    ...
    showModalBottomSheet(context: context, isScrollControlled: true,
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setSheet) => Padding(
          ...
          child: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
            const Text('Report Incident', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
            const SizedBox(height: 12),
            DropdownButtonFormField<IncidentType>(
              initialValue: type,
              decoration: const InputDecoration(labelText: 'Incident Type', border: OutlineInputBorder()),
              items: IncidentType.values.map((t) => DropdownMenuItem(value: t, child: Text(t.name))).toList(),
              onChanged: (v) { if (v != null) setSheet(() => type = v); },
            ),
            const SizedBox(height: 8),
            ...
            const SizedBox(height: 8),
            TextField(controller: repCtl, decoration: const InputDecoration(labelText: 'Reported By', border: OutlineInputBorder())),
            const SizedBox(height: 12),
            SizedBox(
              width: double.infinity,
              child: ElevatedButton(
    ...
    showModalBottomSheet(context: context, isScrollControlled: true,
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setSheet) => Padding(
          ...
          child: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
            const Text('Edit Incident', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
            const SizedBox(height: 12),
            DropdownButtonFormField<IncidentType>(
              initialValue: type,
              decoration: const InputDecoration(labelText: 'Incident Type', border: OutlineInputBorder()),
            ...
            DropdownButtonFormField<IncidentStatus>(
              ...
              items: IncidentStatus.values.map((s) => DropdownMenuItem(value: s, child: Text(s.name))).toList(),
              onChanged: (v) { if (v != null) setSheet(() => status = v); },
            ),
            const SizedBox(height: 8),
            TextField(controller: notesCtl, decoration: const InputDecoration(labelText: 'Action Taken / Notes', border: OutlineInputBorder()), maxLines: 2),
            const SizedBox(height: 12),
            SizedBox(
              width: double.infinity,
              child: ElevatedButton(
                ...
                onPressed: () {
                  SecurityAuditStore.updateIncident(inc.id, SecurityIncident(
                    ...
                    dateResolved: status == IncidentStatus.resolved ? (inc.dateResolved ?? DateTime.now()) : null,
                    resolvedBy: status == IncidentStatus.resolved ? (inc.resolvedBy ?? (repCtl.text.isEmpty ? null : repCtl.text)) : null,
                    notes: notesCtl.text.isEmpty ? null : notesCtl.text,
                  ));
                  Navigator.pop(ctx);
                  onChange();
      ...
      body: Column(children: [
        ...
        Expanded(
          child: passes.isEmpty
              ...
              : ListView.builder(padding: const EdgeInsets.all(8), itemCount: passes.length, itemBuilder: (ctx, i) {
                  final v = passes[i];
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                        if (v.active)
                          RoleGate(requiredPermission: Permission.manageVisitorPasses, child: IconButton(
                            icon: Icon(Icons.logout_rounded, color: _primary),
                            tooltip: 'Check Out',
                            onPressed: () {
                              SecurityAuditStore.updateVisitorPass(v.id, VisitorPass(id: v.id, visitorName: v.visitorName, purpose: v.purpose, hostName: v.hostName, badgeNumber: v.badgeNumber, notes: v.notes, checkIn: v.checkIn, checkOut: DateTime.now(), active: false));
                              onChange();
                            },
                          )),
                        RoleGate(requiredPermission: Permission.manageVisitorPasses, child: PopupMenuButton<String>(
                          itemBuilder: (_) => [
                            const PopupMenuItem(value: 'detail', child: Text('View')),
                            const PopupMenuItem(value: 'edit', child: Text('Edit')),
                            const PopupMenuItem(value: 'delete', child: Text('Delete', style: TextStyle(color: AppColors.red))),
                          ...
                          onSelected: (v2) {
                            if (v2 == 'detail') {
                              showDialog(context: context, builder: (ctx) => AlertDialog(
                                ...
                                content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
                                  _row('Visitor', v.visitorName),
                                  _row('Purpose', v.purpose),
                                  _row('Host', v.hostName),
                                  if (v.badgeNumber != null) _row('Badge', v.badgeNumber!),
                                  _row('Check In', v.checkIn.toIso8601String().substring(0, 16)),
                                  if (v.checkOut != null) _row('Check Out', v.checkOut!.toIso8601String().substring(0, 16)),
                                  if (v.notes != null) _row('Notes', v.notes!),
                                  _row('Status', v.active ? 'On-Site' : 'Checked Out'),
                                ]),
                                actions: [TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Close'))],
                              ));
                            }
                            if (v2 == 'edit') { _editVisitor(context, v); }
                            if (v2 == 'delete') { SecurityAuditStore.removeVisitorPass(v.id); onChange(); }
                          },
                        )),
                      ]),
                    ),
  ...
  void _showForm(BuildContext context) {
    ...
    showModalBottomSheet(context: context, isScrollControlled: true,
      ...
      builder: (ctx) => Padding(
        ...
        child: SingleChildScrollView(
          child: Column(mainAxisSize: MainAxisSize.min, children: [
          const Text('Issue Visitor Pass', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
          const SizedBox(height: 12),
          TextField(controller: nameCtl, decoration: const InputDecoration(labelText: 'Visitor Name', border: OutlineInputBorder())),
          const SizedBox(height: 8),
          TextField(controller: purposeCtl, decoration: const InputDecoration(labelText: 'Purpose of Visit', border: OutlineInputBorder())),
          const SizedBox(height: 8),
          TextField(controller: hostCtl, decoration: const InputDecoration(labelText: 'Host Name', border: OutlineInputBorder())),
          const SizedBox(height: 8),
          TextField(controller: badgeCtl, decoration: const InputDecoration(labelText: 'Badge Number', border: OutlineInputBorder())),
          const SizedBox(height: 8),
            ...
            child: ElevatedButton(
              ...
              onPressed: () {
                ...
                SecurityAuditStore.addVisitorPass(VisitorPass(
                  id: 'sa_${DateTime.now().millisecondsSinceEpoch}',
                  visitorName: nameCtl.text, purpose: purposeCtl.text,
    ...
    showModalBottomSheet(context: context, isScrollControlled: true,
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setSheet) => Padding(
          ...
          child: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
            const Text('Edit Visitor Pass', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
            const SizedBox(height: 12),
            TextField(controller: nameCtl, decoration: const InputDecoration(labelText: 'Visitor Name', border: OutlineInputBorder())),
            const SizedBox(height: 8),
            TextField(controller: purposeCtl, decoration: const InputDecoration(labelText: 'Purpose of Visit', border: OutlineInputBorder())),
              ...
              child: ElevatedButton(
                ...
                onPressed: () {
                  ...
                  SecurityAuditStore.updateVisitorPass(v.id, VisitorPass(
                    ...
                    notes: notesCtl.text.isEmpty ? null : notesCtl.text,
                    checkIn: v.checkIn,
                    checkOut: active ? v.checkOut : (v.checkOut ?? DateTime.now()),
                    active: active,
                  ));
                  Navigator.pop(ctx);
                  onChange();
...
class _ShiftTab extends StatelessWidget {
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                Container(
                  ...
                  child: Icon(_shiftIcon(active.shift), color: _primary, size: 20),
                ),
              ...
              SizedBox(
                width: double.infinity,
                child: RoleGate(requiredPermission: Permission.manageShiftHandover, child: ElevatedButton.icon(
  ...
  IconData _shiftIcon(ShiftType s) {
    switch (s) {
      case ShiftType.morning: return Icons.wb_sunny_rounded;
      case ShiftType.afternoon: return Icons.wb_cloudy_rounded;
      case ShiftType.night: return Icons.nights_stay_rounded;
    }
  }
    ...
    showModalBottomSheet(context: context, isScrollControlled: true,
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setSheet) => Padding(
          ...
          child: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
            const Text('Start Shift', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
            const SizedBox(height: 12),
            DropdownButtonFormField<ShiftType>(
              initialValue: shift,
              decoration: const InputDecoration(labelText: 'Shift', border: OutlineInputBorder()),
              items: ShiftType.values.map((t) => DropdownMenuItem(value: t, child: Text('${t.name} Shift'))).toList(),
              onChanged: (v) { if (v != null) setSheet(() => shift = v); },
            ),
            const SizedBox(height: 8),
            TextField(controller: nameCtl, decoration: const InputDecoration(labelText: 'Staff Name', border: OutlineInputBorder())),
            const SizedBox(height: 8),
            TextField(controller: notesCtl, decoration: const InputDecoration(labelText: 'Handover Notes', border: OutlineInputBorder()), maxLines: 2),
            const SizedBox(height: 12),
            SizedBox(
              width: double.infinity,
              child: ElevatedButton(
          ...
          child: ElevatedButton(
            ...
            onPressed: () {
              SecurityAuditStore.updateShift(shift.id, ShiftHandover(
                ...
                openedAt: shift.openedAt, closedAt: DateTime.now(),
                notes: notesCtl.text.isEmpty ? null : notesCtl.text,
                isActive: false,
              ));
              Navigator.pop(ctx);
