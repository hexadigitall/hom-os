...
class _ProcurementTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final openOrders = orders
        .where((p) =>
            p.status != ProcurementStatus.delivered &&
            p.status != ProcurementStatus.cancelled)
        .toList();
    final totalSpend = orders
        .where((p) => p.status == ProcurementStatus.delivered)
                  ...
                  itemBuilder: (ctx, i) {
                    ...
                    return Card(
                      ...
                      leading: CircleAvatar(
                        backgroundColor:
                            po.status == ProcurementStatus.delivered
                                ? _primary
                                : po.status == ProcurementStatus.cancelled
                                    ...
                                    : po.status == ProcurementStatus.approved
                                        ? AppColors.blue
                                        : AppColors.grey500,
                        child: Text(po.status.name[0].toUpperCase(),
                            style: const TextStyle(
                                color: AppColors.white,
                                fontWeight: FontWeight.w700)),
                      ),
                      title: Text(po.vendorName,
                          style: const TextStyle(fontWeight: FontWeight.w600)),
                      subtitle: Text(
                          '${po.items}  •  ₦${_fmt(po.amount)}  •  ${po.orderDate.toIso8601String().substring(0, 10)}',
                      ...
                      trailing: RoleGate(
                          ...
                          child: PopupMenuButton<String>(
                            itemBuilder: (_) => [
                              ...
                              if (po.status != ProcurementStatus.delivered &&
                                  po.status != ProcurementStatus.cancelled)
                                const PopupMenuItem(
                                    value: 'cancel',
                                    child: Text('Cancel',
                            ...
                            onSelected: (v) {
                              if (v == 'edit') {
                                ...
                                BackOfficeStore.updateProcurement(
                                    po.id,
                                    ProcurementOrder(
                                        id: po.id,
                                ...
                                ExpenditureStore.add(ExpenditureRecord(
                                  id: 'po_${po.id}',
                                  date: DateTime.now(),
                                  category: ExpenditureCategory.procurement,
                                  subcategory: 'Stock',
                                  description:
                                      'PO: ${po.items} from ${po.vendorName}',
                                  amount: po.amount,
                                ...
                                BackOfficeStore.removeProcurement(po.id);
                                onChange();
                              }
                            },
                          )),
...
class _PayrollTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final pending =
        records.where((p) => p.status == PayrollStatus.pending).toList();
    return Scaffold(
      body: Column(children: [
        Container(
          padding: const EdgeInsets.all(12),
          ...
          child: Wrap(children: [
            ...
            HomMetricCard(
                label: 'Total Paid',
                value: '₦${_fmt(BackOfficeStore.totalPaid)}',
                color: AppColors.blue),
          ]),
        ),
        Expanded(
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (ctx, i) {
                    ...
                    return Card(
                        child: ListTile(
                      leading: CircleAvatar(
                        backgroundColor: r.status == PayrollStatus.paid
                            ? _primary
                            : AppColors.orange,
                        child: Text(r.staffName[0],
                            style: const TextStyle(
                                color: AppColors.white,
                                fontWeight: FontWeight.w700)),
                      ),
                      title: Text(r.staffName,
                          style: const TextStyle(fontWeight: FontWeight.w600)),
                      subtitle: Text(
                          '${r.department}  •  Gross: ₦${_fmt(r.grossPay)}  •  Net: ₦${_fmt(r.netPay)}  •  Period: ${r.periodStart.toIso8601String().substring(0, 10)}',
                          style: const TextStyle(fontSize: 11)),
                      trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                        Text('₦${_fmt(r.netPay)}',
                            style: TextStyle(
                                fontWeight: FontWeight.w700,
                                color: r.status == PayrollStatus.paid
                                    ? _primary
                                    : AppColors.orange,
                                fontSize: 12)),
                        ...
                        RoleGate(
                            ...
                            child: PopupMenuButton<String>(
                              ...
                              onSelected: (v) {
                                ...
                                if (v == 'delete') {
                                  BackOfficeStore.removePayroll(r.id);
                                  onChange();
                                }
                              },
                            )),
  ...
  void _editForm(BuildContext context, PayrollRecord r) {
    ...
    final basicCtl =
        TextEditingController(text: r.basicSalary.toStringAsFixed(0));
    final allowCtl =
        TextEditingController(text: r.allowances.toStringAsFixed(0));
    final deductCtl =
        TextEditingController(text: r.deductions.toStringAsFixed(0));
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              Container(
                  ...
                  decoration: BoxDecoration(
                      color: AppColors.grey300,
                      borderRadius: BorderRadius.circular(2))),
              const Text('Edit Payroll Record',
                  style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
              const SizedBox(height: 12),
              TextField(
                  controller: nameCtl,
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () {
                    if (nameCtl.text.isEmpty) return;
                    final basic =
                        double.tryParse(basicCtl.text) ?? r.basicSalary;
                    final allow =
                        double.tryParse(allowCtl.text) ?? r.allowances;
                    final deduct =
                        double.tryParse(deductCtl.text) ?? r.deductions;
                    final gross = basic + allow;
                    final taxConfig = BackOfficeStore.taxConfig;
                    final payeTax = gross * 0.07;
                    final pensEmp = basic * taxConfig.pensionEmployeeRate / 100;
...
class _TaxConfigTab extends StatelessWidget {
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            Container(
              ...
              child: Text(t.active ? 'Active' : 'Inactive',
                  style: TextStyle(
                      fontSize: 10,
                      fontWeight: FontWeight.w700,
                      color: t.active ? _primary : AppColors.grey500)),
            ),
          ]),
          const SizedBox(height: 16),
          ...
          _configRow(
              'LGA Dev. Levy', '${t.lgaDevelopmentLevy.toStringAsFixed(1)}%'),
          const Divider(),
          _configRow('Pension (Employee)',
              '${t.pensionEmployeeRate.toStringAsFixed(1)}%'),
          _configRow('Pension (Employer)',
              '${t.pensionEmployerRate.toStringAsFixed(1)}%'),
          const SizedBox(height: 16),
          Row(children: [
            Expanded(
                child: RoleGate(
                  ...
                  onPressed: () => showDialog(
                      ...
                      builder: (ctx) => AlertDialog(
                            ...
                            actions: [
                              ...
                              TextButton(
                                  onPressed: () {
                                    BackOfficeStore.resetTaxConfig();
                                    Navigator.pop(ctx);
                                    onChange();
                                  },
  ...
  Widget _configRow(String label, String value) => Padding(
        ...
        child:
          ...
          Flexible(
              child: Text(label,
                  style:
                      const TextStyle(fontSize: 13, color: AppColors.grey500))),
          const SizedBox(width: 8),
          Flexible(
              child: Text(value,
                  style: const TextStyle(
  ...
  void _editForm(BuildContext context, TaxConfiguration t) {
    ...
    final lgaCtl =
        TextEditingController(text: t.lgaDevelopmentLevy.toStringAsFixed(1));
    final pensEmpCtl =
        TextEditingController(text: t.pensionEmployeeRate.toStringAsFixed(1));
    final pensEmprCtl =
        TextEditingController(text: t.pensionEmployerRate.toStringAsFixed(1));
            ...
            child: SingleChildScrollView(
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                Row(children: [
                  Expanded(
                      child: TextField(
                          controller: pensEmpCtl,
                          decoration: const InputDecoration(
                              labelText: 'Pension (Emp %)',
                              border: OutlineInputBorder()),
                          keyboardType: TextInputType.number)),
                  const SizedBox(width: 8),
                  Expanded(
                      child: TextField(
                          controller: pensEmprCtl,
                          decoration: const InputDecoration(
                              labelText: 'Pension (Empr %)',
                              border: OutlineInputBorder()),
                          keyboardType: TextInputType.number)),
                ]),
                const SizedBox(height: 12),
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: () {
                      BackOfficeStore.updateTaxConfig(TaxConfiguration(
                        ...
                        lgaDevelopmentLevy: double.tryParse(lgaCtl.text) ??
                            t.lgaDevelopmentLevy,
                        pensionEmployeeRate: double.tryParse(pensEmpCtl.text) ??
                            t.pensionEmployeeRate,
                        pensionEmployerRate:
                            double.tryParse(pensEmprCtl.text) ??
                                t.pensionEmployerRate,
