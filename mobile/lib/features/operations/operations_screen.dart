...
class _OperationsScreenState extends State<OperationsScreen> with SingleTickerProviderStateMixin {
      ...
      body: Column(children: [
        TabBar(
          ...
          tabs: const [
            ...
            Tab(text: 'Night Audit', icon: Icon(Icons.nights_stay_rounded, size: 18)),
            Tab(text: 'Housekeeping', icon: Icon(Icons.cleaning_services_rounded, size: 18)),
          ],
        ),
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setSheet) => Padding(
          ...
          child: SingleChildScrollView(
            ...
            const Text('Add Daily Revenue', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
            const SizedBox(height: 12),
            TextField(controller: revenueCtl, decoration: const InputDecoration(labelText: 'Total Revenue (₦)', border: OutlineInputBorder()), keyboardType: TextInputType.number),
            const SizedBox(height: 8),
            TextField(controller: roomsSoldCtl, decoration: const InputDecoration(labelText: 'Rooms Sold', border: OutlineInputBorder()), keyboardType: TextInputType.number),
            const SizedBox(height: 8),
            Row(children: [
              Expanded(child: TextField(controller: roomsAvailCtl, decoration: const InputDecoration(labelText: 'Rooms Available', border: OutlineInputBorder()), keyboardType: TextInputType.number)),
              const SizedBox(width: 12),
              Expanded(child: TextField(controller: walkInsCtl, decoration: const InputDecoration(labelText: 'Walk-ins', border: OutlineInputBorder()), keyboardType: TextInputType.number)),
            ]),
            const SizedBox(height: 8),
            TextField(
              decoration: const InputDecoration(labelText: 'Date', border: OutlineInputBorder(), suffixIcon: Icon(Icons.calendar_today_rounded, size: 18)),
              controller: TextEditingController(text: '${selectedDate.year}-${selectedDate.month.toString().padLeft(2, '0')}-${selectedDate.day.toString().padLeft(2, '0')}'),
              readOnly: true,
              onTap: () async {
                final picked = await showDatePicker(context: ctx, initialDate: selectedDate, firstDate: DateTime(2024), lastDate: DateTime.now().add(const Duration(days: 1)));
              ...
              child: ElevatedButton(
                ...
                onPressed: () {
                  ...
                  OperationsStore.addRevenue(DailyRevenue(
                    date: selectedDate, roomsAvailable: int.tryParse(roomsAvailCtl.text) ?? 12,
                    roomsSold: sold, walkIns: int.tryParse(walkInsCtl.text) ?? 0,
                    totalRevenue: rev,
                  ));
                  Navigator.pop(ctx);
                  _onChange();
  ...
  void _addCashDrop(BuildContext context) {
    if (SecurityAuditStore.isTodayLocked) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Today is locked — cannot add cash drops')));
      return;
    }
    ...
    showModalBottomSheet(context: context, isScrollControlled: true,
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setSheet) => Padding(
          ...
          child: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
            const Text('Log Cash Drop', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
            const SizedBox(height: 12),
            ...
            const SizedBox(height: 8),
            Row(children: [
              Expanded(child: TextField(controller: expectedCtl, decoration: const InputDecoration(labelText: 'Expected (₦)', border: OutlineInputBorder()), keyboardType: TextInputType.number)),
              const SizedBox(width: 12),
              Expanded(child: TextField(controller: actualCtl, decoration: const InputDecoration(labelText: 'Actual (₦)', border: OutlineInputBorder()), keyboardType: TextInputType.number)),
            ]),
            const SizedBox(height: 8),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                onPressed: () {
                  ...
                  if (exp <= 0) return;
                  final status = act == exp ? CashDropStatus.matched : act < exp ? CashDropStatus.mismatched : CashDropStatus.mismatched;
...
class _RevparTab extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final revs = OperationsStore.revenuesForMonth(DateTime.now().year, DateTime.now().month);
    ...
    return ListView(padding: const EdgeInsets.all(12), children: [
      ...
      SizedBox(
        ...
        child: Row(crossAxisAlignment: CrossAxisAlignment.end, children: [
          ...revs.map((r) {
            final maxRev = revs.fold<double>(0, (m, r) => r.totalRevenue > m ? r.totalRevenue : m);
      ...
      ...revs.takeLast(14).toList().reversed.map((r) {
        ...
        final idx = OperationsStore.revenues.indexWhere((x) => x.date.day == r.date.day && x.date.month == r.date.month);
        return Card(margin: const EdgeInsets.only(bottom: 4), child: InkWell(
          ...
          child: Padding(
            ...
            child: Row(children: [
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  Text('${r.date.day}/${r.date.month}', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w700)),
                  const SizedBox(width: 8),
                  Text(_fmt(r.totalRevenue), style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: _primaryGreen)),
                ]),
                const SizedBox(height: 2),
  ...
  void _editRevenue(BuildContext context, DailyRevenue r, int? idx) {
    final revCtl = TextEditingController(text: r.totalRevenue.toStringAsFixed(0));
    final roomsSoldCtl = TextEditingController(text: r.roomsSold.toString());
    final roomsAvailCtl = TextEditingController(text: r.roomsAvailable.toString());
    ...
    showModalBottomSheet(context: context, isScrollControlled: true,
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setSheet) => Padding(
          ...
          child: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
            const Text('Edit Daily Revenue', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
            const SizedBox(height: 12),
            TextField(controller: revCtl, decoration: const InputDecoration(labelText: 'Total Revenue (₦)', border: OutlineInputBorder()), keyboardType: TextInputType.number),
            const SizedBox(height: 8),
            Row(children: [
              Expanded(child: TextField(controller: roomsSoldCtl, decoration: const InputDecoration(labelText: 'Rooms Sold', border: OutlineInputBorder()), keyboardType: TextInputType.number)),
              const SizedBox(width: 12),
              Expanded(child: TextField(controller: roomsAvailCtl, decoration: const InputDecoration(labelText: 'Rooms Available', border: OutlineInputBorder()), keyboardType: TextInputType.number)),
            ]),
            const SizedBox(height: 8),
            TextField(controller: walkInsCtl, decoration: const InputDecoration(labelText: 'Walk-ins', border: OutlineInputBorder()), keyboardType: TextInputType.number),
            const SizedBox(height: 12),
            SizedBox(
              width: double.infinity,
              child: ElevatedButton(
                ...
                onPressed: () {
                  ...
                  OperationsStore.updateRevenue(idx, DailyRevenue(
                    date: r.date, roomsAvailable: int.tryParse(roomsAvailCtl.text) ?? r.roomsAvailable,
                    roomsSold: sold, walkIns: int.tryParse(walkInsCtl.text) ?? r.walkIns,
                    totalRevenue: rev,
                  ));
                  Navigator.pop(ctx);
                  onChange();
...
class _NightAuditTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final matched = weekDrops.where((c) => c.status == CashDropStatus.matched).length;
    final mismatched = weekDrops.where((c) => c.status == CashDropStatus.mismatched).length;
    final totalDiscrepancy = weekDrops.fold(0.0, (s, c) => s + c.difference.abs());
  ...
  void _editCashDrop(BuildContext context, CashDrop c) {
    ...
    final expectedCtl = TextEditingController(text: c.expectedAmount.toStringAsFixed(0));
    final actualCtl = TextEditingController(text: c.actualAmount.toStringAsFixed(0));
    ...
    showModalBottomSheet(context: context, isScrollControlled: true,
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setSheet) => Padding(
          ...
          child: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
            const Text('Edit Cash Drop', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
            const SizedBox(height: 12),
            DropdownButtonFormField<String>(
              initialValue: shift,
              decoration: const InputDecoration(labelText: 'Shift', border: OutlineInputBorder()),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                onPressed: () {
                  ...
                  final act = double.tryParse(actualCtl.text) ?? 0;
                  if (exp <= 0) return;
                  final status = act == exp ? CashDropStatus.matched : CashDropStatus.mismatched;
                  OperationsStore.updateCashDrop(c.id, CashDrop(
                    id: c.id, date: c.date, shift: shift,
                    expectedAmount: exp, actualAmount: act,
                    notes: notesCtl.text, status: status,
                  ));
                  Navigator.pop(ctx);
                  onChange();
                },
      ...
      if (byCat.isNotEmpty) ...[
        ...
        ...byCat.entries.map((e) {
          ...
          return Padding(
            ...
            child: Row(children: [
              Flexible(flex: 2, child: Text(e.key, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: c), overflow: TextOverflow.ellipsis)),
              const SizedBox(width: 8),
              Expanded(
                flex: 4,
                child: ClipRRect(
