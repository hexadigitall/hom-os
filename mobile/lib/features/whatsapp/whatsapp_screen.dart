...
class WhatsAppScreen extends StatefulWidget {
  ...
  const WhatsAppScreen({super.key, this.initialTemplateId, this.initialPhone, this.initialVars});
  @override
  State<WhatsAppScreen> createState() => _WhatsAppScreenState();
}

class _WhatsAppScreenState extends State<WhatsAppScreen> with SingleTickerProviderStateMixin {
    ...
    return Scaffold(
      ...
      body: Column(children: [
        ...
        Expanded(child: TabBarView(controller: _tabCtrl, children: [
          _ContactsTab(searchCtrl: _searchCtrl, onChanged: () => setState(() {})),
          _TemplatesTab(initialTemplateId: widget.initialTemplateId, initialPhone: widget.initialPhone, initialVars: widget.initialVars),
          _HistoryTab(),
        ])),
      ]),
  ...
  Widget _dashboardRow(int contacts, int sentToday) {
    return Container(
      ...
      child: Row(children: [
        ...
        const SizedBox(width: 12),
        _stat('${WhatsAppStore.templates.length}', 'Templates', Icons.description_rounded),
      ]),
    );
  }
        ...
        child: SafeArea(
          child: SingleChildScrollView(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(height: 16),
              TextField(controller: nameCtl, decoration: const InputDecoration(labelText: 'Contact Name')),
              const SizedBox(height: 12),
              TextField(controller: phoneCtl, keyboardType: TextInputType.phone, decoration: const InputDecoration(labelText: 'Phone Number', hintText: '0803 123 4567')),
              const SizedBox(height: 12),
              DropdownButtonFormField<String>(
                initialValue: entityType,
                items: const [
                  DropdownMenuItem(value: 'booking', child: Text('Booking Guest')),
                ...
                child: ElevatedButton(
                  onPressed: () {
                    ...
                    WhatsAppStore.addContact(WhatsAppContact(
                      id: WhatsAppStore.genId(), name: nameCtl.text, phone: phoneCtl.text,
                      entityType: WhatsAppStore.parseEntityType(entityType), notes: notesCtl.text,
                    ));
                    Navigator.pop(ctx);
                    setState(() {});
                  },
...
class _ContactsTab extends StatelessWidget {
  ...
  Widget _contactCard(BuildContext context, WhatsAppContact c) {
    ...
    if (q.isNotEmpty && !c.name.toLowerCase().contains(q) && !c.phone.contains(q)) return const SizedBox.shrink();
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (c.entityType != ContactEntityType.other)
                Container(
                  ...
                  decoration: BoxDecoration(color: _entityColor(c.entityType).withValues(alpha: 0.1), borderRadius: BorderRadius.circular(10)),
                  child: Text(_entityLabel(c.entityType), style: TextStyle(fontSize: 9, fontWeight: FontWeight.w700, color: _entityColor(c.entityType))),
                ),
            ]),
          ),
          RoleGate(
        ...
        child: SafeArea(
          child: SingleChildScrollView(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              TextField(controller: nameCtl, decoration: const InputDecoration(labelText: 'Contact Name')),
              const SizedBox(height: 12),
              TextField(controller: phoneCtl, keyboardType: TextInputType.phone, decoration: const InputDecoration(labelText: 'Phone Number')),
              const SizedBox(height: 12),
              DropdownButtonFormField<String>(
                initialValue: entityType,
                items: const [
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: () {
                    if (nameCtl.text.isEmpty || phoneCtl.text.isEmpty) return;
                    c.name = nameCtl.text; c.phone = phoneCtl.text; c.notes = notesCtl.text;
                    c.entityType = WhatsAppStore.parseEntityType(entityType);
  ...
  String _entityLabel(ContactEntityType t) {
    switch (t) {
      case ContactEntityType.booking: return 'Guest';
      case ContactEntityType.staff: return 'Staff';
      case ContactEntityType.vendor: return 'Vendor';
      case ContactEntityType.subscription: return 'Subscription';
      case ContactEntityType.compliance: return 'Compliance';
      case ContactEntityType.other: return 'Other';
    }
  }

  String _entityTypeStr(ContactEntityType t) {
    switch (t) {
      case ContactEntityType.booking: return 'booking';
      case ContactEntityType.staff: return 'staff';
      case ContactEntityType.vendor: return 'vendor';
      case ContactEntityType.subscription: return 'subscription';
      case ContactEntityType.compliance: return 'compliance';
...
class _SendSheetState extends State<_SendSheet> {
  ...
  @override
  Widget build(BuildContext context) {
    final templates = WhatsAppStore.templates.where((t) =>
        t.entityType == null || t.entityType == widget.contact.entityType).toList();
        ...
        child: SingleChildScrollView(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            if (templates.isNotEmpty) ...[
              const Text('Quick Template', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 12)),
              const SizedBox(height: 4),
              SizedBox(
                height: 28,
                child: ListView(scrollDirection: Axis.horizontal, children: templates.map((t) => Padding(
            ...
            SizedBox(
              ...
              child: RoleGate(
                ...
                child: ElevatedButton.icon(
                  onPressed: _msgCtrl.text.isEmpty ? null : () => _send(context),
...
class _TemplatesTab extends StatelessWidget {
  ...
  final String? initialPhone;
  final Map<String, String>? initialVars;
  const _TemplatesTab({this.initialTemplateId, this.initialPhone, this.initialVars});
  ...
  void _previewAndSend(BuildContext context, WhatsAppTemplate t) {
    showModalBottomSheet(
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setSB) {
        ...
        final msgCtl = TextEditingController(text: initialVars != null
                  ...
                  child: RoleGate(
                    ...
                    child: ElevatedButton.icon(
                      onPressed: () async {
                        ...
                        final raw = phoneCtl.text.replaceAll(RegExp(r'[^\d+]'), '');
        ...
        child: SafeArea(
          child: SingleChildScrollView(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(height: 16),
              TextField(controller: phoneCtl, keyboardType: TextInputType.phone, decoration: const InputDecoration(labelText: 'Recipient Phone')),
              const SizedBox(height: 12),
              TextField(controller: msgCtl, maxLines: 4, decoration: const InputDecoration(labelText: 'Message Body', border: OutlineInputBorder(borderRadius: BorderRadius.all(Radius.circular(12))))),
              const SizedBox(height: 12),
              DropdownButtonFormField<MessageStatus>(
                initialValue: selectedStatus,
                items: const [
                  DropdownMenuItem(value: MessageStatus.sent, child: Text('Sent')),
                  DropdownMenuItem(value: MessageStatus.delivered, child: Text('Delivered')),
                  DropdownMenuItem(value: MessageStatus.failed, child: Text('Failed')),
                ],
              ...
              SizedBox(
                ...
                child: RoleGate(
                  ...
                  child: ElevatedButton(
                    onPressed: () {
                      if (phoneCtl.text.isEmpty || msgCtl.text.isEmpty) return;
