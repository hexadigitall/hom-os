      ...
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: const [
            ...
            Tab(text: 'Lost & Found', icon: Icon(Icons.search_rounded, size: 15)),
...
class _TasksTab extends StatelessWidget {
      ...
      body: Column(children: [
        ...
        Expanded(
          child: pending.isEmpty
              ...
              : ListView.builder(padding: const EdgeInsets.all(8), itemCount: pending.length, itemBuilder: (ctx, i) {
                  final t = pending[i];
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                        RoleGate(requiredPermission: Permission.assignRoomAttendants, child: IconButton(
                          icon: Icon(Icons.check_circle, color: _primary),
                          onPressed: () {
                            HousekeepingStore.updateTask(t.id, HousekeepingTask(id: t.id, roomNumber: t.roomNumber, assignedTo: t.assignedTo, notes: t.notes, priority: t.priority, scheduledDate: t.scheduledDate, completedDate: DateTime.now(), completed: true));
                            onChange();
                          },
                        )),
                        RoleGate(requiredPermission: Permission.assignRoomAttendants, child: PopupMenuButton<String>(
                          ...
                          onSelected: (v) {
                            ...
                            if (v == 'delete') { HousekeepingStore.removeTask(t.id); onChange(); }
                          },
                        )),
                      ]),
  ...
  void _showTaskForm(BuildContext context, {HousekeepingTask? task}) {
    ...
    HousekeepingPriority priority = task?.priority ?? HousekeepingPriority.routine;
        ...
        builder: (ctx, setSheet) => Padding(
          ...
          child: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              Text(task == null ? 'New Task' : 'Edit Task', style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
              ...
              DropdownButtonFormField<String>(
                ...
                items: ['101', '102', '103', '201', '202', 'Lobby', 'Pool', 'Restaurant'].map((r) => DropdownMenuItem(value: r, child: Text(r))).toList(),
                onChanged: (v) { if (v != null) setSheet(() => room = v); },
              ),
              const SizedBox(height: 8),
              DropdownButtonFormField<HousekeepingPriority>(
                initialValue: priority,
                decoration: const InputDecoration(labelText: 'Priority', border: OutlineInputBorder()),
                items: HousekeepingPriority.values.map((p) => DropdownMenuItem(value: p, child: Text(p.name))).toList(),
              ...
              ListTile(
                ...
                onTap: () async {
                  final picked = await showDatePicker(context: context, initialDate: date, firstDate: DateTime.now().subtract(const Duration(days: 1)), lastDate: DateTime.now().add(const Duration(days: 30)));
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () {
                    ...
                    final t = HousekeepingTask(
                      id: task?.id ?? 'hk_${DateTime.now().millisecondsSinceEpoch}',
                      roomNumber: room, assignedTo: assignCtl.text, notes: notesCtl.text.isEmpty ? null : notesCtl.text,
                      priority: priority, scheduledDate: date,
                    );
                    if (task != null) { HousekeepingStore.updateTask(task.id, t); } else { HousekeepingStore.addTask(t); }
                    Navigator.pop(ctx);
                    onChange();
                  },
...
class _LaundryTab extends StatelessWidget {
              ...
              return Card(child: ListTile(
                ...
                trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                  if (l.chargeAmount != null && l.chargeAmount! > 0)
                    Text('₦${l.chargeAmount!.toStringAsFixed(0)}', style: TextStyle(fontWeight: FontWeight.w700, color: _primary, fontSize: 12)),
                  RoleGate(requiredPermission: Permission.manageLaundry, child: PopupMenuButton<String>(
                    ...
                    onSelected: (v) {
                      ...
                      HousekeepingStore.updateLaundry(l.id, LaundryItem(id: l.id, guestName: l.guestName, roomNumber: l.roomNumber, itemDescription: l.itemDescription, status: next, type: l.type, receivedDate: l.receivedDate, readyDate: next == LaundryStatus.delivered ? DateTime.now() : null, chargeAmount: l.chargeAmount));
                      onChange();
                    },
                  )),
                ]),
    ...
    showModalBottomSheet(context: context, isScrollControlled: true,
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setSheet) => Padding(
          ...
          child: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              const Text('Log Laundry', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
              const SizedBox(height: 12),
              TextField(controller: guestCtl, decoration: const InputDecoration(labelText: 'Guest Name', border: OutlineInputBorder())),
              const SizedBox(height: 8),
              TextField(controller: roomCtl, decoration: const InputDecoration(labelText: 'Room Number', border: OutlineInputBorder())),
              const SizedBox(height: 8),
              TextField(controller: descCtl, decoration: const InputDecoration(labelText: 'Items', border: OutlineInputBorder()), maxLines: 2),
              const SizedBox(height: 8),
              DropdownButtonFormField<LaundryType>(
                initialValue: type,
                decoration: const InputDecoration(labelText: 'Service Type', border: OutlineInputBorder()),
                items: LaundryType.values.map((t) => DropdownMenuItem(value: t, child: Text(t.name))).toList(),
                onChanged: (v) { if (v != null) setSheet(() => type = v); },
              ),
              if (type != LaundryType.selfService) ...[
                const SizedBox(height: 8),
                TextField(controller: amtCtl, decoration: const InputDecoration(labelText: 'Charge (₦)', border: OutlineInputBorder()), keyboardType: TextInputType.number),
              ],
              const SizedBox(height: 12),
              SizedBox(
                width: double.infinity,
                child: ElevatedButton(
                  ...
                  onPressed: () {
                    ...
                    final item = LaundryItem(
                      ...
                      guestName: guestCtl.text, roomNumber: roomCtl.text,
                      itemDescription: descCtl.text, type: type,
  ...
  void _showLaundryEditForm(BuildContext context, {required LaundryItem laundry}) {
    ...
    final amtCtl = TextEditingController(text: laundry.chargeAmount?.toStringAsFixed(0) ?? '');
    ...
    showModalBottomSheet(context: context, isScrollControlled: true,
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setSheet) => Padding(
          ...
          child: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              const Text('Edit Laundry', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
              const SizedBox(height: 12),
              TextField(controller: guestCtl, decoration: const InputDecoration(labelText: 'Guest Name', border: OutlineInputBorder())),
              const SizedBox(height: 8),
              TextField(controller: roomCtl, decoration: const InputDecoration(labelText: 'Room Number', border: OutlineInputBorder())),
...
class _LostFoundTab extends StatelessWidget {
              ...
              : ListView.builder(padding: const EdgeInsets.all(8), itemCount: items.length, itemBuilder: (ctx, i) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      subtitle: Text('${lf.locationFound}  •  ${lf.category.name}  •  ${lf.foundDate.toIso8601String().substring(0, 10)}', style: const TextStyle(fontSize: 11)),
                      trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                        if (!lf.returned)
                          RoleGate(requiredPermission: Permission.manageLostAndFound, child: IconButton(
                            ...
                            onPressed: () {
                              HousekeepingStore.updateLostFound(lf.id, LostFoundItem(id: lf.id, itemName: lf.itemName, foundBy: lf.foundBy, locationFound: lf.locationFound, guestName: lf.guestName, notes: lf.notes, category: lf.category, foundDate: lf.foundDate, claimedDate: DateTime.now(), returned: true));
                              onChange();
                            },
                          )),
                        RoleGate(requiredPermission: Permission.manageLostAndFound, child: IconButton(
                        ...
                        RoleGate(requiredPermission: Permission.manageLostAndFound, child: PopupMenuButton<String>(
                          ...
                          onSelected: (v) {
                            ...
                            if (v == 'delete') { HousekeepingStore.removeLostFound(lf.id); onChange(); }
                          },
                        )),
                      ]),
                    ),
  ...
  void _showDetail(BuildContext context, LostFoundItem lf) {
    showDialog(context: context, builder: (ctx) => AlertDialog(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _row('Date', lf.foundDate.toIso8601String().substring(0, 10)),
    ...
    showModalBottomSheet(context: context, isScrollControlled: true,
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setSheet) => Padding(
          ...
          child: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              const Text('Log Found Item', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
              const SizedBox(height: 12),
              TextField(controller: nameCtl, decoration: const InputDecoration(labelText: 'Item Name', border: OutlineInputBorder())),
              const SizedBox(height: 8),
              TextField(controller: foundCtl, decoration: const InputDecoration(labelText: 'Found By', border: OutlineInputBorder())),
              const SizedBox(height: 8),
              TextField(controller: locCtl, decoration: const InputDecoration(labelText: 'Location Found', border: OutlineInputBorder())),
              const SizedBox(height: 8),
              DropdownButtonFormField<LostFoundCategory>(
                initialValue: cat,
                decoration: const InputDecoration(labelText: 'Category', border: OutlineInputBorder()),
                items: LostFoundCategory.values.map((c) => DropdownMenuItem(value: c, child: Text(c.name))).toList(),
                onChanged: (v) { if (v != null) setSheet(() => cat = v); },
              ),
              const SizedBox(height: 8),
              TextField(controller: guestCtl, decoration: const InputDecoration(labelText: 'Guest Name (if known)', border: OutlineInputBorder())),
              const SizedBox(height: 8),
              TextField(controller: notesCtl, decoration: const InputDecoration(labelText: 'Notes', border: OutlineInputBorder()), maxLines: 2),
              const SizedBox(height: 12),
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () {
                    ...
                    final item = LostFoundItem(
                      id: 'hk_${DateTime.now().millisecondsSinceEpoch}',
                      itemName: nameCtl.text, foundBy: foundCtl.text,
                      locationFound: locCtl.text, guestName: guestCtl.text.isEmpty ? null : guestCtl.text,
                      notes: notesCtl.text.isEmpty ? null : notesCtl.text, category: cat,
                    );
  ...
  void _showLostFoundEditForm(BuildContext context, {required LostFoundItem item}) {
    ...
    showModalBottomSheet(context: context, isScrollControlled: true,
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setSheet) => Padding(
          ...
          child: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              const Text('Edit Lost & Found Item', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
              const SizedBox(height: 12),
              TextField(controller: nameCtl, decoration: const InputDecoration(labelText: 'Item Name', border: OutlineInputBorder())),
              const SizedBox(height: 8),
              TextField(controller: locCtl, decoration: const InputDecoration(labelText: 'Location Found', border: OutlineInputBorder())),
              ...
              const SizedBox(height: 8),
              ListTile(
                title: Text('Date Found: ${foundDate.toIso8601String().substring(0, 10)}'),
                trailing: const Icon(Icons.calendar_today),
                onTap: () async {
                  final picked = await showDatePicker(context: context, initialDate: foundDate, firstDate: DateTime(2020), lastDate: DateTime.now());
              ...
              if (returned) ...[
                const SizedBox(height: 8),
                TextField(controller: claimCtl, decoration: const InputDecoration(labelText: 'Claimed By', border: OutlineInputBorder())),
              ],
              const SizedBox(height: 8),
              TextField(controller: guestCtl, decoration: const InputDecoration(labelText: 'Guest Name', border: OutlineInputBorder())),
              const SizedBox(height: 8),
              TextField(controller: notesCtl, decoration: const InputDecoration(labelText: 'Description / Notes', border: OutlineInputBorder()), maxLines: 2),
              const SizedBox(height: 12),
              SizedBox(
                width: double.infinity,
                child: ElevatedButton(
                  ...
                  onPressed: () {
                    ...
                    final updated = LostFoundItem(
                      id: item.id,
                      itemName: nameCtl.text, foundBy: item.foundBy,
                      ...
                      category: cat, foundDate: foundDate,
                      claimedDate: returned ? (item.claimedDate ?? DateTime.now()) : null,
                      returned: returned,
                    );
...
class _LinenTab extends StatelessWidget {
                    ...
                    onTap: () {
                      showDialog(context: context, builder: (ctx) => AlertDialog(
                        ...
                        content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
                          ...
                          if (l.roomNumber != null) _row('Room', l.roomNumber!),
                          if (l.replacementCost != null) _row('Replacement Cost', '₦${l.replacementCost!.toStringAsFixed(0)}'),
                          if (l.notes != null) _row('Notes', l.notes!),
                        ]),
                        actions: [TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Close'))],
                      ));
                    },
    ...
    showModalBottomSheet(context: context, isScrollControlled: true,
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setSheet) => Padding(
          ...
          child: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              const Text('Log Linen Damage', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
              const SizedBox(height: 12),
              TextField(controller: nameCtl, decoration: const InputDecoration(labelText: 'Item Name', border: OutlineInputBorder())),
              const SizedBox(height: 8),
              Row(children: [
                Expanded(child: DropdownButtonFormField<LinenCategory>(
                  initialValue: cat,
                  decoration: const InputDecoration(labelText: 'Category', border: OutlineInputBorder()),
                  items: LinenCategory.values.map((c) => DropdownMenuItem(value: c, child: Text(c.name))).toList(),
                  onChanged: (v) { if (v != null) setSheet(() => cat = v); },
                )),
                const SizedBox(width: 8),
                Expanded(child: DropdownButtonFormField<LinenCondition>(
                  initialValue: condition,
                  decoration: const InputDecoration(labelText: 'Condition', border: OutlineInputBorder()),
                  items: LinenCondition.values.map((c) => DropdownMenuItem(value: c, child: Text(c.name))).toList(),
                  onChanged: (v) { if (v != null) setSheet(() => condition = v); },
                )),
              ]),
              const SizedBox(height: 8),
              Row(children: [
                Expanded(child: TextField(controller: qtyCtl, decoration: const InputDecoration(labelText: 'Quantity', border: OutlineInputBorder()), keyboardType: TextInputType.number)),
                const SizedBox(width: 8),
                Expanded(child: TextField(controller: costCtl, decoration: const InputDecoration(labelText: 'Unit Cost (₦)', border: OutlineInputBorder()), keyboardType: TextInputType.number)),
              ]),
              const SizedBox(height: 8),
              TextField(controller: roomCtl, decoration: const InputDecoration(labelText: 'Room (optional)', border: OutlineInputBorder())),
              const SizedBox(height: 8),
              TextField(controller: notesCtl, decoration: const InputDecoration(labelText: 'Notes', border: OutlineInputBorder()), maxLines: 2),
              const SizedBox(height: 12),
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () {
                    ...
                    final item = LinenDamage(
                      id: 'hk_${DateTime.now().millisecondsSinceEpoch}',
                      itemName: nameCtl.text, roomNumber: roomCtl.text.isEmpty ? null : roomCtl.text,
                      notes: notesCtl.text.isEmpty ? null : notesCtl.text,
  ...
  void _showLinenEditForm(BuildContext context, {required LinenDamage item}) {
    ...
    final costCtl = TextEditingController(text: item.replacementCost?.toStringAsFixed(0) ?? '');
    final notesCtl = TextEditingController(text: item.notes ?? '');
    ...
    showModalBottomSheet(context: context, isScrollControlled: true,
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setSheet) => Padding(
          ...
          child: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              const Text('Edit Linen Damage', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
              const SizedBox(height: 12),
              TextField(controller: nameCtl, decoration: const InputDecoration(labelText: 'Item Name', border: OutlineInputBorder())),
              const SizedBox(height: 8),
              Row(children: [
