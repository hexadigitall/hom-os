...
class _SubscriptionsScreenState extends State<SubscriptionsScreen> {
  void _add({Subscription? existing}) {
    ...
    final amtCtl = TextEditingController(text: existing?.amount.toString() ?? '');
        ...
        child: SafeArea(
          child: SingleChildScrollView(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(height: 16),
              TextField(controller: nameCtl, decoration: const InputDecoration(labelText: 'Subscription Name', hintText: 'e.g. DSTV Premium Business')),
              const SizedBox(height: 12),
              TextField(controller: provCtl, decoration: const InputDecoration(labelText: 'Provider', hintText: 'e.g. Multichoice Nigeria')),
              const SizedBox(height: 12),
              TextField(controller: catCtl, decoration: const InputDecoration(labelText: 'Category', hintText: 'e.g. TV, Internet, License')),
              const SizedBox(height: 12),
              ...
              const SizedBox(height: 12),
              TextField(controller: contactCtl, decoration: const InputDecoration(labelText: 'Contact Info', hintText: 'Phone / Email')),
              const SizedBox(height: 12),
              TextField(controller: notesCtl, maxLines: 2, decoration: const InputDecoration(labelText: 'Notes')),
              const SizedBox(height: 8),
              CheckboxListTile(
                title: const Text('Auto-log to Expenditure', style: TextStyle(fontSize: 13), overflow: TextOverflow.ellipsis),
                value: autoLog,
              ...
              SizedBox(
                ...
                child: RoleGate(
                  ...
                  child: ElevatedButton(
                    onPressed: () {
                      ...
                      if (existing != null) {
                        ...
                        final sub = Subscription(
                          id: SubscriptionStore.genId(), name: nameCtl.text,
                          provider: provCtl.text, category: catCtl.text,
                          amount: amt, billingCycle: cycle, startDate: start,
                          contactInfo: contactCtl.text, notes: notesCtl.text,
                          autoLogExpenditure: autoLog,
                        );
                        SubscriptionStore.add(sub);
                        if (autoLog) {
                          ExpenditureStore.add(ExpenditureRecord(
                            id: 'sub_${SubscriptionStore.genId()}', date: DateTime.now(),
                            category: ExpenditureCategory.administrative,
                            subcategory: 'Subscription — ${sub.category}',
                            description: '${sub.name} (${sub.provider}) — ${sub.billingCycle.label}',
                            amount: sub.amount,
                            vendor: sub.provider,
                          ));
                        }
                    ...
                    child: Text(existing != null ? 'Save Changes' : 'Add Subscription'),
                  ),
                ),
              ),
            ]),
      ...
      appBar: AppBar(
        ...
        actions: [
          Padding(
            ...
            child: Center(
              child: Text('₦${_fmtShort(monthlyTotal)}/mo', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 13, color: AppColors.primary)),
            ),
          ),
        ],
      ),
