...
class _EngineeringScreenState extends State<EngineeringScreen> with SingleTickerProviderStateMixin {
  late TabController _tabController;

  @override
  void initState() { super.initState(); _tabController = TabController(length: 6, vsync: this); }
  @override
      ...
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: const [
            ...
            Tab(text: 'Fuel & Tanks', icon: Icon(Icons.local_gas_station_rounded, size: 15)),
            Tab(text: 'Grid & Cost', icon: Icon(Icons.bolt_rounded, size: 15)),
            Tab(text: 'Water Treatment', icon: Icon(Icons.water_drop_rounded, size: 15)),
          ],
        ),
      ),
...
class _DashboardTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final totalGenCost = EngineeringStore.generators.fold(0.0, (s, g) => s + g.currentRunHours * g.fuelRateLph * 1.2);
        ...
        Card(child: Padding(
          ...
          child: Column(children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text('Grid (${tariff?.bandName ?? 'N/A'})', style: const TextStyle(fontWeight: FontWeight.w600)),
              Text('₦${_fmtShort(tariff?.monthlyCost ?? 0)}/mo', style: const TextStyle(fontWeight: FontWeight.w800)),
            ]),
...
class _GeneratorsTab extends StatelessWidget {
                  ...
                  itemBuilder: (ctx, i) {
                    ...
                    return Card(child: Column(children: [
                      ListTile(
                        ...
                        trailing: RoleGate(requiredPermission: Permission.trackGeneratorRunHours, child: PopupMenuButton<String>(
                          onSelected: (v) {
                            if (v == 'edit') _showForm(context, gen: g);
                            if (v == 'toggle') {
                              final next = g.status == GeneratorStatus.running ? GeneratorStatus.idle : GeneratorStatus.running;
                              EngineeringStore.updateGenerator(g.id, Generator(id: g.id, name: g.name, model: g.model, location: g.location, capacityKva: g.capacityKva, currentRunHours: g.currentRunHours, lastOilChangeHours: g.lastOilChangeHours, currentLoadKva: g.currentLoadKva, status: next, lastServiceDate: g.lastServiceDate));
                              onChange();
                            }
                            if (v == 'delete') { EngineeringStore.removeGenerator(g.id); onChange(); }
                          },
                          itemBuilder: (_) => [
  ...
  Widget _loadBar(String label, double pct, Color color) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        ...
        const Spacer(),
        Text('${pct.toStringAsFixed(0)}%', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: color)),
      ]),
  ...
  void _showForm(BuildContext context, {Generator? gen}) {
    ...
    final kvaCtl = TextEditingController(text: gen?.capacityKva.toStringAsFixed(0) ?? '');
    final loadCtl = TextEditingController(text: gen?.currentLoadKva.toStringAsFixed(0) ?? '0');
    final hoursCtl = TextEditingController(text: gen?.currentRunHours.toStringAsFixed(0) ?? '');
    final oilCtl = TextEditingController(text: gen?.lastOilChangeHours.toStringAsFixed(0) ?? '0');
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setSheet) => SingleChildScrollView(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            const SizedBox(height: 12),
            TextField(controller: nameCtl, decoration: const InputDecoration(labelText: 'Generator Name', border: OutlineInputBorder())),
            homField(Row(children: [
              Expanded(child: TextField(controller: modelCtl, decoration: const InputDecoration(labelText: 'Model', border: OutlineInputBorder()))),
              const SizedBox(width: 8),
              Expanded(child: TextField(controller: kvaCtl, decoration: const InputDecoration(labelText: 'Capacity (kVA)', border: OutlineInputBorder()), keyboardType: TextInputType.number)),
            ])),
            TextField(controller: locCtl, decoration: const InputDecoration(labelText: 'Location', border: OutlineInputBorder())),
            homField(Row(children: [
              Expanded(child: TextField(controller: loadCtl, decoration: const InputDecoration(labelText: 'Current Load (kVA)', border: OutlineInputBorder()), keyboardType: TextInputType.number)),
              const SizedBox(width: 8),
              Expanded(child: TextField(controller: hoursCtl, decoration: const InputDecoration(labelText: 'Run Hours', border: OutlineInputBorder()), keyboardType: TextInputType.number)),
            ])),
            homField(Row(children: [
              Expanded(child: TextField(controller: oilCtl, decoration: const InputDecoration(labelText: 'Oil Change (hrs)', border: OutlineInputBorder()), keyboardType: TextInputType.number)),
              const SizedBox(width: 8),
              Expanded(child: DropdownButtonFormField<GeneratorStatus>(
                initialValue: status,
                decoration: const InputDecoration(labelText: 'Status', border: OutlineInputBorder()),
                items: GeneratorStatus.values.map((s) => DropdownMenuItem(value: s, child: Text(s.name))).toList(),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                onPressed: () {
                  ...
                  final g = Generator(
                    id: gen?.id ?? 'g_${DateTime.now().millisecondsSinceEpoch}',
                    name: nameCtl.text, model: modelCtl.text.isEmpty ? null : modelCtl.text,
                    location: locCtl.text.isEmpty ? null : locCtl.text,
                    capacityKva: kva, currentRunHours: double.tryParse(hoursCtl.text) ?? 0,
                    lastOilChangeHours: double.tryParse(oilCtl.text) ?? 0,
                    currentLoadKva: double.tryParse(loadCtl.text) ?? 0,
                    status: status,
                  );
                  if (gen != null) { EngineeringStore.updateGenerator(gen.id, g); } else { EngineeringStore.addGenerator(g); }
                  Navigator.pop(ctx);
                  onChange();
                },
...
class _FuelTanksTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final dieselCost = logs.where((f) => f.fuelType.name == 'diesel').fold(0.0, (s, f) => s + f.cost);
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        SizedBox(
          width: double.infinity,
          child: RoleGate(requiredPermission: Permission.trackFuelDeliveryCycles, child: ElevatedButton.icon(
  ...
  void _showDipDetail(BuildContext context, TankDipLog d) {
    showDialog(context: context, builder: (ctx) => AlertDialog(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _det('Dip Reading', '${d.dipReadingCm.toStringAsFixed(0)} cm'),
  ...
  Widget _det(String label, String value, [Color? color]) => Padding(
    ...
    child: Row(children: [
      ...
      Flexible(child: Text(value, style: TextStyle(fontSize: 13, color: color))),
    ]),
  );

  void _showDipForm(BuildContext context, {TankDipLog? dip}) {
    final readingCtl = TextEditingController(text: dip?.dipReadingCm.toStringAsFixed(0) ?? '');
    final capacityCtl = TextEditingController(text: dip?.tankCapacityL.toStringAsFixed(0) ?? '5000');
        ...
        builder: (ctx, setSheet) => SingleChildScrollView(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            DropdownButtonFormField<String>(
              ...
              decoration: const InputDecoration(labelText: 'Tank', border: OutlineInputBorder()),
              items: ['Main Diesel Tank', 'Generator Day Tank', 'Petrol Storage', 'LPG Cylinder'].map((t) => DropdownMenuItem(value: t, child: Text(t))).toList(),
              onChanged: (v) { if (v != null) setSheet(() { tankName = v; if (v == 'Generator Day Tank') { capacityCtl.text = '500'; } else if (v == 'Petrol Storage') { capacityCtl.text = '1000'; } else { capacityCtl.text = '5000'; } }); },
            ),
            const SizedBox(height: 12),
            homField(Row(children: [
              Expanded(child: TextField(controller: readingCtl, decoration: const InputDecoration(labelText: 'Dip Reading (cm)', border: OutlineInputBorder()), keyboardType: TextInputType.number)),
              const SizedBox(width: 8),
              Expanded(child: TextField(controller: capacityCtl, decoration: const InputDecoration(labelText: 'Tank Capacity (L)', border: OutlineInputBorder()), keyboardType: TextInputType.number)),
            ])),
            TextField(controller: performedCtl, decoration: const InputDecoration(labelText: 'Performed By', border: OutlineInputBorder())),
            const SizedBox(height: 12),
              ...
              child: ElevatedButton(
                ...
                onPressed: () {
                  ...
                  if (lastDip != null && lastDip.tankName == tankName) {
                    ...
                    final daysSince = DateTime.now().difference(lastDip.date).inDays.toDouble();
                    expected = prevVol - (EngineeringStore.totalRunHours * 12 * daysSince / 30);
                  }
                  final entry = TankDipLog(
                    id: dip?.id ?? 'dip_${DateTime.now().millisecondsSinceEpoch}',
                    date: dip?.date ?? DateTime.now(), tankName: tankName,
                    dipReadingCm: reading, tankCapacityL: cap,
                    calculatedVolumeL: vol, expectedVolumeL: expected,
                    performedBy: performedCtl.text.isEmpty ? null : performedCtl.text,
...
class _GridCostTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final dieselLogs = app.HOMData.fuelLogs.where((f) => f.fuelType.name == 'diesel').toList();
    final totalDieselL = dieselLogs.fold(0.0, (s, f) => s + f.quantity);
    final dieselCost = dieselLogs.fold(0.0, (s, f) => s + f.cost);
    final dieselLph = EngineeringStore.totalRunHours > 0 ? totalDieselL / EngineeringStore.totalRunHours : 0;
    final dieselPerKwh = dieselLph > 0 && EngineeringStore.totalCapacity > 0 ? (dieselCost / totalDieselL * dieselLph) / EngineeringStore.totalCapacity : 0;
    ...
    return Scaffold(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        ...tariffs.map((t) => Card(
          ...
          child: ListTile(
            ...
            trailing: Row(mainAxisSize: MainAxisSize.min, children: [
              ...
              RoleGate(requiredPermission: Permission.trackGridTariffUsage, child: PopupMenuButton<String>(
                onSelected: (v) {
                  if (v == 'edit') _showTariffForm(context, tariff: t);
                  if (v == 'delete') { EngineeringStore.removeTariff(t.id); onChange(); }
                },
                itemBuilder: (_) => [
                  const PopupMenuItem(value: 'edit', child: Text('Edit')),
                  const PopupMenuItem(value: 'delete', child: Text('Delete', style: TextStyle(color: Colors.red))),
        ...
        Card(child: Padding(
          padding: const EdgeInsets.all(16),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Grid Electricity', style: TextStyle(fontWeight: FontWeight.w700)),
            Text('Active: ${active?.bandName ?? 'None'}  •  ₦${active?.costPerKwh.toStringAsFixed(0) ?? 'N/A'}/kWh  •  ${active?.hoursPerDay.toStringAsFixed(0) ?? 'N/A'}h/day',
                style: const TextStyle(fontSize: 12)),
            const Divider(),
            const Text('Diesel Generation', style: TextStyle(fontWeight: FontWeight.w700)),
            Text('Avg consumption: ${dieselLph.toStringAsFixed(1)} L/hr  •  Est. cost: ₦${dieselPerKwh.toStringAsFixed(0)}/kWh',
                style: const TextStyle(fontSize: 12)),
            const Divider(),
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
  ...
  void _showTariffForm(BuildContext context, {GridTariffConfig? tariff}) {
    ...
    final hoursCtl = TextEditingController(text: (tariff?.hoursPerDay ?? 16).toStringAsFixed(0));
    final costCtl = TextEditingController(text: (tariff?.costPerKwh ?? 100).toStringAsFixed(0));
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setSheet) => SingleChildScrollView(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            const SizedBox(height: 12),
            TextField(controller: descCtl, decoration: const InputDecoration(labelText: 'Description', border: OutlineInputBorder())),
            const SizedBox(height: 12),
            Row(children: [
              Expanded(child: TextField(controller: hoursCtl, decoration: const InputDecoration(labelText: 'Hours/Day', border: OutlineInputBorder()), keyboardType: TextInputType.number)),
              const SizedBox(width: 12),
              Expanded(child: TextField(controller: costCtl, decoration: const InputDecoration(labelText: 'Cost (₦/kWh)', border: OutlineInputBorder()), keyboardType: TextInputType.number)),
            ]),
            const SizedBox(height: 12),
            DropdownButtonFormField<GridBand>(
              initialValue: band,
              decoration: const InputDecoration(labelText: 'Band', border: OutlineInputBorder()),
              items: GridBand.values.map((b) => DropdownMenuItem(value: b, child: Text('Band ${b.name.toUpperCase()}'))).toList(),
              onChanged: (v) { if (v != null) setSheet(() => band = v); },
            ),
            const SizedBox(height: 12),
            SizedBox(
              width: double.infinity,
              child: ElevatedButton(
                ...
                onPressed: () {
                  ...
                  if (labelCtl.text.isEmpty || hours <= 0 || cost <= 0) return;
                  final entry = GridTariffConfig(
                    id: tariff?.id ?? 't_${DateTime.now().millisecondsSinceEpoch}',
                    band: band, hoursPerDay: hours, costPerKwh: cost,
                    label: labelCtl.text, description: descCtl.text,
                  );
...
class _MaintenanceTab extends StatelessWidget {
                        ...
                        trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                          if (!t.completed)
                            RoleGate(requiredPermission: Permission.managePreventativeMaintenance, child: IconButton(
                              ...
                              onPressed: () {
                                EngineeringStore.updateTask(t.id, MaintenanceTask(id: t.id, equipmentName: t.equipmentName, description: t.description, assignedTo: t.assignedTo, equipmentType: t.equipmentType, priority: t.priority, scheduledDate: t.scheduledDate, completedDate: DateTime.now(), completed: true));
                                onChange();
                              },
                            )),
                          RoleGate(requiredPermission: Permission.managePreventativeMaintenance, child: IconButton(
  ...
  void _showForm(BuildContext context, {MaintenanceTask? task}) {
    ...
    DateTime dueDate = task?.scheduledDate ?? DateTime.now().add(const Duration(days: 7));
    EquipmentType equipType = task?.equipmentType ?? EquipmentType.generator;
    MaintenancePriority priority = task?.priority ?? MaintenancePriority.routine;
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setSheet) => SingleChildScrollView(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            const SizedBox(height: 12),
            TextField(controller: equipCtl, decoration: const InputDecoration(labelText: 'Equipment Name', border: OutlineInputBorder())),
            const SizedBox(height: 12),
            TextField(controller: descCtl, decoration: const InputDecoration(labelText: 'Description', border: OutlineInputBorder()), maxLines: 2),
            const SizedBox(height: 12),
            TextField(controller: assignCtl, decoration: const InputDecoration(labelText: 'Assigned To', border: OutlineInputBorder())),
            const SizedBox(height: 12),
            Row(children: [
              Expanded(child: DropdownButtonFormField<EquipmentType>(
                initialValue: equipType,
                decoration: const InputDecoration(labelText: 'Type', border: OutlineInputBorder()),
                items: EquipmentType.values.map((e) => DropdownMenuItem(value: e, child: Text(e.name))).toList(),
                onChanged: (v) { if (v != null) setSheet(() => equipType = v); },
              )),
              const SizedBox(width: 12),
              Expanded(child: DropdownButtonFormField<MaintenancePriority>(
                initialValue: priority,
                decoration: const InputDecoration(labelText: 'Priority', border: OutlineInputBorder()),
                items: MaintenancePriority.values.map((p) => DropdownMenuItem(value: p, child: Text(p.name))).toList(),
                onChanged: (v) { if (v != null) setSheet(() => priority = v); },
              )),
            ]),
            const SizedBox(height: 12),
            ListTile(
              ...
              child: ElevatedButton(
                ...
                onPressed: () {
                  ...
                  final updated = MaintenanceTask(
                    id: task?.id ?? 'm_${DateTime.now().millisecondsSinceEpoch}',
                    equipmentName: equipCtl.text, description: descCtl.text,
                    assignedTo: assignCtl.text, equipmentType: equipType,
                    priority: priority, scheduledDate: dueDate,
...
class _WaterTab extends StatelessWidget {
  ...
  void _showDetail(BuildContext context, WaterTreatmentLog w) {
    showDialog(context: context, builder: (ctx) => AlertDialog(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        _detailRow('Date', w.date.toIso8601String().substring(0, 10)),
        _detailRow('pH Level', '${w.phLevel}'),
        _detailRow('Chlorine', w.chlorineLevel?.toStringAsFixed(1) ?? 'N/A'),
        _detailRow('TDS', '${w.tdsLevel?.toStringAsFixed(0) ?? 'N/A'} ppm'),
        _detailRow('Chemical', w.chemicalUsed ?? 'N/A'),
        _detailRow('Dosage', w.chemicalDosageMl != null ? '${w.chemicalDosageMl!.toStringAsFixed(0)} ml' : 'N/A'),
        _detailRow('Next Due', w.nextScheduledDate?.toIso8601String().substring(0, 10) ?? 'N/A'),
  ...
  Widget _detailRow(String label, String value) => Padding(
    padding: const EdgeInsets.symmetric(vertical: 2),
    child: Row(children: [Text('$label: ', style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13)), Flexible(child: Text(value, style: const TextStyle(fontSize: 13)))]),
  );
  ...
  void _showForm(BuildContext context, {WaterTreatmentLog? log}) {
    ...
    final dosageCtl = TextEditingController(text: log?.chemicalDosageMl?.toStringAsFixed(0) ?? '');
    final phCtl = TextEditingController(text: log?.phLevel.toStringAsFixed(1) ?? '7.0');
    final chlorineCtl = TextEditingController(text: log?.chlorineLevel?.toStringAsFixed(1) ?? '');
    final tdsCtl = TextEditingController(text: log?.tdsLevel?.toStringAsFixed(0) ?? '');
        ...
        builder: (ctx, setSheet) => SingleChildScrollView(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            DropdownButtonFormField<String>(
              ...
              items: ['Borehole', 'RO Plant', 'STP', 'Swimming Pool'].map((s) => DropdownMenuItem(value: s, child: Text(s))).toList(),
              onChanged: (v) { if (v != null) setSheet(() => source = v); },
            ),
            const SizedBox(height: 12),
            DropdownButtonFormField<String>(
              initialValue: action,
              decoration: const InputDecoration(labelText: 'Action', border: OutlineInputBorder()),
              items: ['backwash', 'chemicalDosing', 'filterClean', 'sampleTest'].map((a) => DropdownMenuItem(value: a, child: Text(a))).toList(),
              onChanged: (v) { if (v != null) setSheet(() => action = v); },
            ),
            const SizedBox(height: 12),
            Row(children: [
              Expanded(child: TextField(decoration: const InputDecoration(labelText: 'pH Level', border: OutlineInputBorder()), keyboardType: TextInputType.number, controller: phCtl)),
              const SizedBox(width: 12),
              Expanded(child: TextField(decoration: const InputDecoration(labelText: 'Chlorine (mg/L)', border: OutlineInputBorder()), keyboardType: TextInputType.number, controller: chlorineCtl)),
            ]),
            const SizedBox(height: 12),
            homField(Row(children: [
              Expanded(child: TextField(controller: chemCtl, decoration: const InputDecoration(labelText: 'Chemical Used', border: OutlineInputBorder()))),
              const SizedBox(width: 12),
              Expanded(child: TextField(controller: dosageCtl, decoration: const InputDecoration(labelText: 'Dosage (ml)', border: OutlineInputBorder()), keyboardType: TextInputType.number)),
            ])),
            homField(Row(children: [
              Expanded(child: TextField(decoration: const InputDecoration(labelText: 'TDS (ppm)', border: OutlineInputBorder()), keyboardType: TextInputType.number, controller: tdsCtl)),
              const SizedBox(width: 12),
              Expanded(child: TextField(controller: performedCtl, decoration: const InputDecoration(labelText: 'Performed By', border: OutlineInputBorder()))),
            ])),
            const SizedBox(height: 12),
            TextField(controller: notesCtl, decoration: const InputDecoration(labelText: 'Notes', border: OutlineInputBorder()), maxLines: 2),
            const SizedBox(height: 12),
              ...
              child: ElevatedButton(
                ...
                onPressed: () {
                  final entry = WaterTreatmentLog(
                    ...
                    date: log?.date ?? DateTime.now(), source: source,
                    ...
                    performedBy: performedCtl.text.isEmpty ? null : performedCtl.text,
                    notes: notesCtl.text.isEmpty ? null : notesCtl.text,
                    chemicalUsed: chemCtl.text.isEmpty ? null : chemCtl.text,
                    chemicalDosageMl: double.tryParse(dosageCtl.text),
                    nextScheduledDate: log?.nextScheduledDate ?? DateTime.now().add(const Duration(days: 7)),
                  );
                  if (log != null) {
